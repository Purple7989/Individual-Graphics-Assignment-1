...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		// Vehicle Models
		MeshResource::Sptr regularCarMesh = ResourceManager::CreateAsset<MeshResource>("FutureCar1.obj");
		...
		// Vehicle Textures
		Texture2D::Sptr    regularCarTex = ResourceManager::CreateAsset<Texture2D>("textures/FutureCarTex.png");
		Texture2D::Sptr    pickupTruckTex = ResourceManager::CreateAsset<Texture2D>("textures/PickupTruckTexV2.png");
		...
		scene->SetColorLUT(lutWarm);
		scene->SetColorLUTCool(lutCool);
		...
		// Vehicle Materials
		Material::Sptr RegularCarMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			RegularCarMaterial->Name = "RegularCar Material";
			RegularCarMaterial->Set("u_Material.Diffuse", regularCarTex);
			RegularCarMaterial->Set("u_Material.Shininess", 0.5f);
			RegularCarMaterial->Set("s_ToonTerm", toonLut);
			RegularCarMaterial->Set("u_Material.Steps", 8);
		}
		Material::Sptr PickupTruckMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			PickupTruckMaterial->Name = "PickupTruck Material";
			PickupTruckMaterial->Set("u_Material.Diffuse", pickupTruckTex);
			PickupTruckMaterial->Set("u_Material.Shininess", 0.5f);
			PickupTruckMaterial->Set("s_ToonTerm", toonLut);
			PickupTruckMaterial->Set("u_Material.Steps", 8);
		}
		Material::Sptr SemiTruckMaterial = ResourceManager::CreateAsset<Material>(basicShader);
		{
			SemiTruckMaterial->Name = "SemiTruck Material";
			SemiTruckMaterial->Set("u_Material.Diffuse", semiTruckTex);
			SemiTruckMaterial->Set("u_Material.Shininess", 0.5f);
			SemiTruckMaterial->Set("s_ToonTerm", toonLut);
			SemiTruckMaterial->Set("u_Material.Steps", 8);
		}

		// Create some lights for our scene
		scene->Lights.resize(3);
		scene->Lights[0].Position = glm::vec3(-4.5f, 10.0f, 5.0f);
		...
		scene->Lights[2].Position = glm::vec3(10.5f, -10.0f, 8.0f);
		...
		GameObject::Sptr RegularCar = scene->CreateGameObject("RegularCar");
		{
			// Set position in the scene
			RegularCar->SetPostion(glm::vec3(-4.5f, 10.0f, 3.0f));
			RegularCar->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			// Add some behaviour that relies on the physics body
			// Don't forget to register behavior in application.cpp -> Run()
			RegularCar->Add<VehicleMover>();

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = RegularCar->Add<RenderComponent>();
			renderer->SetMesh(regularCarMesh);
			renderer->SetMaterial(RegularCarMaterial);

			// Add a dynamic rigid body to this object
			RigidBody::Sptr physics = RegularCar->Add<RigidBody>(RigidBodyType::Kinematic);
			// For Wall Jump Colliders, X = Left/Right Y = towards/away, z = Up/Down
			ICollider::Sptr CarCollider = physics->AddCollider(BoxCollider::Create(glm::vec3(0.100f, 0.200f, 0.800f)));
			CarCollider->SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));

			/*
			// Example of a trigger that interacts with static and kinematic bodies as well as dynamic bodies
			TriggerVolume::Sptr trigger = RegularCar->Add<TriggerVolume>();
			trigger->SetFlags(TriggerTypeFlags::Statics | TriggerTypeFlags::Kinematics);
			trigger->AddCollider(ConvexMeshCollider::Create());
			
			RegularCar->Add<TriggerVolumeEnterBehaviour>();
			*/
		...
		GameObject::Sptr PickupTruck = scene->CreateGameObject("PickupTruck");
		{
			// Set position in the scene
			PickupTruck->SetPostion(glm::vec3(-4.5f, 30.0f, 2.0f));
			PickupTruck->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			// Add some behaviour that relies on the physics body
			PickupTruck->Add<VehicleMover>();

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = PickupTruck->Add<RenderComponent>();
			renderer->SetMesh(pickupTruckMesh);
			renderer->SetMaterial(PickupTruckMaterial);

			// Add a dynamic rigid body to this object
			RigidBody::Sptr physics = PickupTruck->Add<RigidBody>(RigidBodyType::Kinematic);
			// For Wall Jump Colliders, X = Left/Right Y = towards/away, z = Up/Down
			ICollider::Sptr CarCollider = physics->AddCollider(BoxCollider::Create(glm::vec3(0.100f, 0.200f, 0.800f)));
			CarCollider->SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));
		}
		GameObject::Sptr SemiTruck = scene->CreateGameObject("SemiTruck");
		{
			// Set position in the scene
			SemiTruck->SetPostion(glm::vec3(10.5f, -10.0f, 2.0f));
			SemiTruck->SetRotation(glm::vec3(90.0f, 0.0f, 180.0f));

			// Add some behaviour that relies on the physics body
			SemiTruck->Add<VehicleMover>();

			// Create and attach a renderer for the monkey
			RenderComponent::Sptr renderer = SemiTruck->Add<RenderComponent>();
			renderer->SetMesh(semiTruckMesh);
			renderer->SetMaterial(SemiTruckMaterial);

			// Add a dynamic rigid body to this object
			RigidBody::Sptr physics = SemiTruck->Add<RigidBody>(RigidBodyType::Kinematic);
			// For Wall Jump Colliders, X = Left/Right Y = towards/away, z = Up/Down
			ICollider::Sptr CarCollider = physics->AddCollider(BoxCollider::Create(glm::vec3(0.100f, 0.200f, 0.800f)));
			CarCollider->SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));
